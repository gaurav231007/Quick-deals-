...
ADMIN_USER_IDS = set()
for _part in ADMIN_IDS_STR.split(","):
    _part = _part.strip()
    if _part:
        try:
            ADMIN_USER_IDS.add(int(_part))
        except ValueError:
            logging.warning(f"Ignoring invalid admin id in ADMIN_USER_ID env var: '{_part}'")
# Kept for any code that still wants "the one primary admin" (e.g. old payment-notification fallback)
ADMIN_USER_ID = next(iter(ADMIN_USER_IDS)) if ADMIN_USER_IDS else None
    ...
    if ADMIN_USER_IDS:
        ...
        for admin_id in ADMIN_USER_IDS:
            try:
                ...
                logging.error(f"Error forwarding support message to admin {admin_id}: {e}")
        ...
        if ADMIN_USER_IDS:
            ...
            for admin_id in ADMIN_USER_IDS:
                try:
                    ...
                    logging.error(f"Error notifying admin {admin_id} of payment: {e}")
